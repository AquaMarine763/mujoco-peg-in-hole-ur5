...
- Multi-geometry status:
  ...
  - Next same-shape data step should be a guarded rollout scale-up, not another raw-policy DAgger run. Start with `512` to `1024` samples/profile, inspect geometry balance and success-sourced sample rate, then decide whether to train an approach adapter/model update.
