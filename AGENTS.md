...
- Multi-geometry status:
  ...
  - v120 adds default-off `guard_final_servo_align_hover_escape_*` wiring in guarded runtime plus eval/demo/inference CLIs. It was added after analyzing `896007`: escape did trigger, but `max_retries=2` exhausted after repeated `square_fast_settle/recover` cycles around `35-40 mm` Z. The current opt-in diagnostic recipe is v119 state handoff plus align-hover escape and `--guard-final-servo-max-retries 4`; targeted seeds `895700/895800/895900/896000`, 10ep each, reached `40/40`, zero collision and zero timeout. Keep this opt-in until broader same-shape/narrow-clearance stress passes.
  - Next same-shape learner/controller step should keep v8 promoted and treat the final-insert adapter and macro recovery as diagnostics only. The useful next move is broader validation of v120 on fixed same-shape and mixed-same-shape narrow-clearance stress, not further broad handoff threshold widening; do not promote v104-v108 or v112-v119 as default runtime behavior.
