...
- Multi-geometry status:
  ...
  - v143 command-level probe raises only `guard_final_servo_square_fast_settle_contact_max` from `6` to `8` on top of v138. This fixed two of the three v139 square-square timeouts: seeds `903500/904500/905500`, 60 episodes each, reached `179/180`, zero collision and one remaining square-square timeout (`903557`). A higher final-servo IK orientation weight probe (`0.12`) did not solve `903557`; do not include it in the candidate.
  - Current conclusion: keep promoted v8 as the stable baseline; treat v143 as the best promising diagnostic, not a stable tag yet. The remaining narrow square-square bottleneck is yaw/clearance instability inside active `square_fast_settle` around `30-40 mm` Z, not retry count, blind high-Z down-push, inactive final-servo rearm, or stronger orientation weight alone. Next work should rerun v143 on seeds `900500/901500/902500`, then target square yaw/clearance-aware phase completion or teacher labels if the one-timeout mode persists.
