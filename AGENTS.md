...
- Multi-geometry status:
  ...
  - v129/v130 adapter/down-action probes on `902500` did not produce a promotable setting. Lowering `final_insert_adapter_max_up_action` from `0.005` to `0.002` improved that seed to `58/60`, zero collisions; `0.001` regressed to `56/60` with one collision. Raising square-fast-settle max down action to `0.0025/0.0030` did not beat `58/60`.
  - Current conclusion: keep promoted v8 as the stable baseline; keep v119/v120/v127 plus reduced adapter-up as diagnostics only. The remaining narrow square-square bottleneck is high-Z `square_fast_settle` under negative tilted-clearance margin where the final-insert adapter often commands upward wall-contact relief and the peg never descends below the success band. Next work should improve square high-Z settle/reinsert policy, not simply raise retries or broaden handoff thresholds.
