...
- Multi-geometry status:
  ...
  - Current conclusion: keep promoted v8 as the stable baseline; treat v138/v139 as a promising diagnostic, not a stable tag yet. The remaining narrow square-square bottleneck is active high-Z `square_fast_settle` phase completion under square clearance/yaw/contact constraints, not retry count, blind high-Z down-push, or inactive final-servo rearm. Next work should target phase completion / teacher labels for those active `square_fast_settle` stalls.
