...
- Multi-geometry status:
  ...
  - Current conclusion: keep promoted v8 as the stable baseline; v138 remains the reproducible diagnostic config, while v143/v144/v145 are not promotion paths. The remaining narrow square-square bottleneck is negative tilted clearance margin inside active `square_fast_settle` around `30-40 mm` Z, not retry count, blind high-Z down-push, inactive final-servo rearm, stronger orientation weight, more tilt-reinsert attempts, or another scalar lift/recenter watchdog. Next work should target teacher labels from stuck traces or a genuinely pose-aware/yaw-capable final-insert controller.
