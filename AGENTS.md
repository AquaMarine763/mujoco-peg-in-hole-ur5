...
- Multi-geometry status:
  ...
  - Current conclusion: keep promoted v8 as the stable baseline; treat v138 as a promising diagnostic, not a stable tag yet. The remaining narrow square-square bottleneck is high-Z `square_fast_settle` under negative tilted-clearance margin where the peg stays wedged above the success band. Next work should validate v138 on a larger fresh matrix and then either tune the square-specific reinsert trigger or improve final-insert teacher labels.
