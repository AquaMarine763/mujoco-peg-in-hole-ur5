...
- Same-shape scaffold smoke:
  ...
  - guarded-deployment same-shape approach collector:
    ...
    - interpretation: default same-shape scaffold is stable under promoted v8, but true narrow-clearance generalization is still bottlenecked by square-square final insertion/tilt/contact stability, not visual approach. The best current opt-in diagnostic is v143 as a small extension of v138: priority-over-fixture + v119 handoff adapter + align-hover escape + retry6 + square-tilt reinsert + `lift_height=0.060` + `square_fast_settle_contact_max=8`. Do not tag it as stable until it is rerun on the earlier v138 seeds `900500/901500/902500`; next work should target the remaining square yaw/clearance instability around `30-40 mm` Z, not retry count, blind high-Z down-push, inactive rearm, or stronger final-servo orientation weight alone.
- Small matrix result:
