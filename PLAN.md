...
- Same-shape scaffold smoke:
  ...
  - guarded-deployment same-shape approach collector:
    ...
    - interpretation: default same-shape scaffold is stable under promoted v8, but true narrow-clearance generalization is still bottlenecked by square-square final insertion/tilt/contact stability, not visual approach. Best stable baseline remains promoted v8; v138 remains the reproducible diagnostic config, while v143/v144/v145 are not promotion paths. The remaining bottleneck is negative tilted clearance margin inside active `square_fast_settle` around `30-40 mm` Z. Since hand-coded lift/recenter watchdogs are now causing seed-specific regressions and collisions, next work should move toward teacher-label/data for stuck traces or a genuinely pose-aware/yaw-capable final-insert controller, not more scalar threshold scans.
- Small matrix result:
