...
- Current version-promotion target: `v0.7.5-square-pose-yaw-align`
- Latest promoted contact-aware approach milestone: `v0.7.5-square-pose-yaw-align` once the promotion commit/tag is pushed
...
- Same-shape scaffold smoke:
  ...
  - guarded-deployment same-shape approach collector:
    ...
    - interpretation: v148 is now the narrow same-shape version-promotion target and directly addresses the square-square negative-margin/yaw stall that translation-only lift/recenter adapters could not solve. Across the original six-seed matrix plus the fresh six-seed v149 gate it has reached `720/720`, zero collision and zero timeout. Promote this as `v0.7.5-square-pose-yaw-align`; after that, the next useful work is stricter stress/fresh-seed regression before broader multi-geometry/randomization changes.
- Small matrix result:
