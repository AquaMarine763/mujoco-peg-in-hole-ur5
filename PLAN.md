...
- Same-shape scaffold smoke:
  ...
  - guarded-deployment same-shape approach collector:
    ...
    - interpretation: default same-shape scaffold is stable under promoted v8, but true narrow-clearance generalization is still bottlenecked by square-square final insertion/tilt/contact stability, not visual approach. The best current opt-in diagnostic is v138/v139: priority-over-fixture + v119 handoff adapter + align-hover escape + retry6 + square-tilt reinsert + `lift_height=0.060`. Do not tag it as stable yet; next work should change square high-Z settle behavior or improve final-insert teacher labels, not scan retry count.
- Small matrix result:
