...
- Same-shape scaffold smoke:
  ...
  - guarded-deployment same-shape approach collector:
    ...
    - interpretation: default same-shape scaffold is stable under promoted v8, but true narrow-clearance generalization is still bottlenecked by square-square final insertion/tilt/contact stability, not visual approach. Best stable baseline remains promoted v8; v138 remains the reproducible diagnostic config, while v143/v144 are not promotion paths. The remaining bottleneck is negative tilted clearance margin inside active `square_fast_settle` around `30-40 mm` Z. Next work should implement a default-off margin/yaw-aware square settle watchdog or collect teacher labels from the stuck traces, not keep scanning retry count, blind high-Z down-push, inactive rearm, stronger final-servo orientation weight, or more tilt-reinsert attempts.
- Small matrix result:
