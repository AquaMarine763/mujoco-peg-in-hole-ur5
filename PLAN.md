...
- Same-shape scaffold smoke:
  ...
  - guarded-deployment same-shape approach collector:
    ...
    - interpretation: default same-shape scaffold is stable under promoted v8, but true narrow-clearance generalization is still bottlenecked by square-square final insertion/tilt/contact stability, not visual approach. The best current opt-in diagnostic is v138: priority-over-fixture + v119 handoff adapter + align-hover escape + retry6 + square-tilt reinsert + `lift_height=0.060`. Do not tag it as stable until it passes a larger fresh-seed matrix; remaining failures are still high-Z `square_fast_settle` timeouts with negative tilted-clearance margin.
- Small matrix result:
