...
- Same-shape scaffold smoke:
  ...
  - guarded-deployment same-shape approach collector:
    ...
    - interpretation: default same-shape scaffold is stable under promoted v8, but true narrow-clearance generalization is bottlenecked by square-square final insertion/tilt/contact stability, not visual approach. The best new safety diagnostic is priority-over-fixture, but it is not a promotion candidate yet. Remaining failures are high-Z `square_fast_settle` cases with negative tilted-clearance margin and too much adapter upward relief; next work should target a square-specific high-Z settle/reinsert policy or better final-insert teacher labels, not broader handoff thresholds.
- Small matrix result:
