...
- Same-shape scaffold smoke:
  ...
  - guarded-deployment same-shape approach collector:
    ...
    - interpretation: default same-shape scaffold is stable under promoted v8, but true narrow-clearance generalization is bottlenecked by square-square final insertion/tilt/contact stability, not visual approach. Simple hand-coded threshold hooks shift failures between timeout and collision. The learned final-insert adapter is useful only as a conservative, default-off diagnostic hook so far; it is not promoted because the best tested improvement is still only targeted `37/40 -> 39/40`. v118/v119 can fix the old `895803` high-Z timeout, but one hard `896007` high-Z/align-hover timeout remains.
- Small matrix result:
