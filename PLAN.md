...
- Same-shape scaffold smoke:
  ...
  - guarded-deployment same-shape approach collector:
    ...
    - interpretation: default same-shape scaffold is stable under promoted v8, but true narrow-clearance generalization is bottlenecked by square-square final insertion/tilt/contact stability, not visual approach. Simple hand-coded threshold hooks shift failures between timeout and collision. The learned final-insert adapter is useful only as a conservative, default-off diagnostic hook so far; it is not promoted because the only tested improvement is targeted `37/40 -> 39/40`, and one seed (`895803`) remains a long near-hole timeout even with the hook active for hundreds of steps.
- Small matrix result:
