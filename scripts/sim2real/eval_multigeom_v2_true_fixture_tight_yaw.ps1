param(
    ...
    [int]$Episodes = 5,
    [string]$Model = "",
    [string]$Config = "configs\sim2real\multigeom_v2_true_fixture_tight_yaw_eval.yaml",
    [string]$ResultDir = "results\sim2real_multigeom_v2_true_fixture_tight_yaw"
)
...
foreach ($Seed in $Seeds) {
    $Prefix = Join-Path $ResultDir ("tight_yaw_{0}_{1}ep_seed{2}" -f $Profile, $Episodes, $Seed)
    $PythonArgs = @(
    ...
    Write-Host "Running sim2real multigeom v2 tight-yaw eval: profile=$Profile seed=$Seed episodes=$Episodes"
    & python @PythonArgs
}
